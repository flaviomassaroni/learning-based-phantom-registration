...
def get_graph_feature(x, k=20):
    ...
    # Batch indexing after flattening
    ...
    feature = x.view(batch_size * num_points, -1)[idx, :] # Collecting Neighbors
    feature = feature.view(batch_size, num_points, k, num_dims)
    x = x.view(batch_size, num_points, 1, num_dims).repeat(1, 1, k, 1)

    # Building DGCNN edge-feature: both neighbor and center informations, the network can learn the local geometry and absolute position 
...
class DGCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = get_graph_feature(x) # Obtaining edge features
        x = F.relu(self.bn1(self.conv1(x)))
...
class SVDHead(nn.Module):
    ...
    def forward(self, *input):
        ...
        # For each point in A, the virtual correlated point in B is the weighted average of all B points, given scores. 
        d_k = src_embedding.size(1)
        scores = torch.matmul(src_embedding.transpose(2, 1).contiguous(), tgt_embedding) / math.sqrt(d_k)
        scores = torch.softmax(scores, dim=2)
        src_corr = torch.matmul(tgt, scores.transpose(2, 1).contiguous())

        # H is the cross covariance matrix between the original point cloud and the virtual correlated points in target.
        ...
        for i in range(src.size(0)):
            ...
            # If det(R) = -1, the SVD produced a reflection, not a rotation. Thus flipping sign of third column of V (forcing det(R) = 1)
            r_det = torch.det(r)
            if r_det < 0:
                u, s, v = torch.svd(H[i])
        ...
        # Trianslation aligning baricenters (t is computed analitically)
